...
const FrontOffice = () => {
  ...
  const fetchRooms = useCallback(async () => {
    try {
      ...
      if (start !== -1 && end !== -1) {
        const cleanData = text.substring(start, end);
        setRooms(JSON.parse(cleanData));
      } else { setRooms([]); }
    } catch (e) { console.error(e); setRooms([]); } 
  ...
  if (loading) return <div style={{padding:'50px', textAlign:'center', fontWeight:'bold'}}>Otel Verileri Yükleniyor...</div>;
    ...
    <div className="module-container">
      <div className="module-header" style={{display:'flex', justifyContent:'space-between', marginBottom:'30px', alignItems:'center'}}>
        ...
        <div style={{display:'flex', gap:'20px', alignItems:'center'}}>
            ...
            <button onClick={() => setShowAddModal(true)} style={{background:'#1a1f36', color:'white', border:'none', padding:'12px 20px', borderRadius:'12px', fontWeight:'800', cursor:'pointer'}}>+ Yeni Oda Ekle</button>
        </div>
      </div>

      <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(220px, 1fr))', gap:'20px'}}>
        {rooms.length === 0 ? (
            <div style={{gridColumn:'1/-1', textAlign:'center', padding:'100px', border:'2px dashed #ddd', borderRadius:'30px', color:'#a0aec0'}}>Henüz oda yok. Sağ üstten ekleyin.</div>
        ) : (
          rooms.map(room => (
            <div key={room.id} style={{
              ...
              <div style={{marginBottom:'20px'}}>
                <small style={{fontWeight:'800', color:'#a0aec0', fontSize:'0.7rem'}}>{room.room_type}</small>
        ...
        <div style={{position:'fixed', top:0, left:0, right:0, bottom:0, background:'rgba(0,0,0,0.8)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:2000, backdropFilter:'blur(5px)'}}>
          <div style={{background:'white', padding:'30px', borderRadius:'25px', width:'300px'}}>
            ...
            {selectedRoom.current_status === 'Temiz' && (
              <div>
                <input type="text" placeholder="Müşteri Adı" value={guestName} onChange={e=>setGuestName(e.target.value)} style={{width:'100%', padding:'12px', marginBottom:'15px', borderRadius:'12px', border:'1px solid #edf2f7'}} />
                <button onClick={() => handleAction('checkin', selectedRoom)} style={{width:'100%', background:'#38a169', color:'white', padding:'12px', borderRadius:'12px', border:'none', fontWeight:'800', cursor:'pointer'}}>GİRİŞ YAP (CHECK-IN)</button>
              </div>
            )}
            {selectedRoom.current_status === 'Dolu' && (
              <button onClick={() => handleAction('checkout', selectedRoom)} style={{width:'100%', background:'#e53e3e', color:'white', padding:'12px', borderRadius:'12px', border:'none', fontWeight:'800', cursor:'pointer'}}>ÇIKIŞ YAP (CHECK-OUT)</button>
            )}
            {selectedRoom.current_status === 'Kirli' && (
              <button onClick={() => handleAction('cleaned', selectedRoom)} style={{width:'100%', background:'#3182ce', color:'white', padding:'12px', borderRadius:'12px', border:'none', fontWeight:'800', cursor:'pointer'}}>TEMİZLENDİ İŞARETLE</button>
            )}
            <button onClick={()=>setSelectedRoom(null)} style={{width:'100%', marginTop:'15px', background:'none', color:'#718096', border:'none', cursor:'pointer', fontWeight:'700'}}>Kapat</button>
          </div>
        </div>
      )}
    </div>
